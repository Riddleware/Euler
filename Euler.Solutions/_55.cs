...
{
    public class _55 : ISolution<int, long>
    {
        ...
        bool IsLychrel(BigInteger l)
        {
            for (BigInteger i = 0; i < 50; i++)
            {
                BigInteger add = l + BigInteger.Parse(Utils.Reverse<BigInteger>(l));
                if (Utils.IsPalindrome<BigInteger>(add))
                    return false;
